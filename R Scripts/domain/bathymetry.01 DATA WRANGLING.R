...
nc_raw <- nc_open("./Objects/data/GEBCO_2019.nc")                       # Open up the netcdf file to see it's raw contents (var names)
...
# Clip longitudes
W <- 80/360 * length(nc_lon) ; E <- 0.5 * length(nc_lon)        # What positions in the vector are -80 & 0 Degrees W?
...
cont <- contourLines(Con_lon, Con_lat, as.matrix(Con_bath), 
  ...
  saveRDS(file = "./Objects/domain/Bathymetry_lines.rds")
...
saveRDS(nc_bath, file = "./Objects/domain/Bathymetry_points.rds")