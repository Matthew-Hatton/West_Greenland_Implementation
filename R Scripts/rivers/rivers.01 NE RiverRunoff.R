...
domains <- readRDS("./Objects/domain/Domains.rds")                                     # Import domain polygon
...
domains <- readRDS("./Objects/domain/Domains.rds") %>%                                 # Import original domain polygon
...
rivers <- future_pmap(all_files, function(file, year, forcing, ssp){
  ...
  nutrients <- rast(file, subds = c("ronh4", "rono3")) %>%                        # Extract nutrient data
    ...
    dplyr::select(-x, -y) %>%                                                            # Don't need spatial information anymore
  ...
  runoff <- rast(file, subds = c("rorunoff","dA")) %>%                            # Extract river runoff
    ...
    dplyr::select(-dA, -x, -y) %>%                                                     # Don't need spatial information anymore
    as.matrix() %>%                                                             # Fast daily summary
