...
total_mixing <- list.files("./Objects/NEMO RAW/NM_vertical boundary/", full.names = T) %>%  # Import
  future_map(readRDS) %>% 
  rbindlist() %>% 
  group_by(Month) %>%                                                                   
...
normal_mixing <- list.files("./Objects/NEMO RAW/NM_vertical boundary/", full.names = T) %>% # Import data
  future_map(readRDS) %>% 
  rbindlist() %>% 
...
saveRDS(normal_mixing, "./Objects/physics/NM/vertical diffusivity.rds")