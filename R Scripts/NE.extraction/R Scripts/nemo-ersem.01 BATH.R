...
packages <- c("MiMeMo.tools", "ncdf4", "sf","nemoRsem")                                # List handy data packages
...
bath_lat <- ncvar_get(raw, varid = "nav_lat") # Pulls latitudes
bath_lon <- ncvar_get(raw, varid = "nav_lon") # Pulls longitudes
bath_bath <- ncvar_get(raw, varid = "bathy_metry") # Pulls bathymetry
nc_close(raw)
